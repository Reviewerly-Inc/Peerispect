...
class PDFOverlayOnOriginal:
    ...
    def create_overlay_on_original(self) -> Path:
        ...
        print("🎨 Creating PDF overlay on original PDF...")
        ...
        for page_num in range(len(reader.pages)):
            ...
            if (page_num + 1) in page_chunks:
                ...
                overlay_canvas.drawString(10, page_height - 15, f"Page {page_num + 1} - {len(chunks_on_page)} chunk positions")
        ...
        output_pdf = self.output_dir / f"{self.pdf_path.stem}_with_chunk_overlay.pdf"
    ...
    def run_full_visualization(self):
        ...
        print(f"🚀 Starting PDF overlay on original for {self.pdf_path.name}")
        print("=" * 60)
...
def main():
    """Main function to create PDF overlay on original."""
    
    # Use the test PDF
    pdf_file = Path("../../outputs/pdfs/Zj8UqVxClT.pdf")
    output_folder = Path("pdf_overlay_on_original_output")
    ...
    overlay_creator = PDFOverlayOnOriginal(pdf_file, output_folder)
